...
`fl_framework` 3/16, `distributed_framework` 4/10, `llm_framework` 3/10,
...
- **Wheel-availability wall resolved, revealing the real remaining work is
  ...
  and now scoped concretely rather than left as a guess**: TensorFlow
  (installs and runs; `onnx2tf`'s own bugs are the new wall). FedScale
  (its TensorFlow import chain resolves, but `Aggregator(args)` wants a
  real ~90-field config revealing FedScale is a full device-simulation/
  executor-launcher platform, not a library a thin adapter wraps).
  FedGraph (its full dependency tree, including native `torch-scatter`/
  `torch-sparse` extensions, now genuinely compiles and installs, but its
  `run_NC`/`run_GC`/`run_LP` entry points want FedGraph's own bundled
  benchmark datasets via their own loader, and `run_NC` hits the same
  transductive/whole-graph wall this project's own GCN generalization
  already found).
